...
1. **Registers Devices**: Exposes an endpoint to onboard devices with unique IDs and human-readable names (`POST /devices`).
2. **Ingests Heartbeats**: Ingests periodic heartbeats containing timestamps, status flags, and telemetry (CPU usage, signal strength, memory usage, battery level) (`POST /devices/:id/heartbeat`).
...
5. **Delete Devices**: Allows operators to decommission/delete devices from the fleet via `DELETE /devices/:id` and via the web dashboard.
6. **Interactive Multi-Device Simulator**: Multi-device simulator driving 5+ concurrent devices, with real-time fault injection to pause any device and watch it transition to `OFFLINE` after 30 seconds.
7. **Live Web Dashboard**: Built-in real-time dashboard served at `http://127.0.0.1:8000/` with live auto-refreshing metrics, second-by-second countdown timers, device status cards, and pulse/delete triggers.
...
The test suite contains **20 automated tests** covering unit logic, exact 30s timeout boundary conditions, error handling, device deletion, and REST API integration.
...
### 6. Delete a Device
```http
DELETE /devices/device-01
```
**Response (`200 OK`):**
```json
{
  "message": "Device 'device-01' deleted successfully",
  "device_id": "device-01"
}
```
...
- **What Used For**:
  ...
  - Crafting the responsive HTML/CSS dashboard for live monitoring with real-time countdown tickers and deletion controls.
...
- **Personally Verified Before Submitting**:
  - Manually executed all 20 Jest and Supertest automated tests, verifying 100% pass rate.
  - Started the Node.js server, verified device registration, heartbeat updates, 30s timeout transitions, deletion API, and live dashboard auto-sync.
